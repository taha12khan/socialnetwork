          ...
          if(isset($_SESSION['Choice'])) {
            if($_SESSION['Choice'] == 8) {
              echo '<div class="alert alert-warning">
                <strong><i class="fa fa-graduation-cap" aria-hidden="true"></i> Nopes! </strong> Entered email is not Registered, <br> Please check the Email and retry.
              </div>';
            }
            session_destroy();
          }
           ?>
          <form action="Forget Password Authontication.php" method="POST">
            <!--Form Starts here-->
            <div class="form-group input-group">
              <div class="input-group-prepend">
                <span class="input-group-text"><i class="fa fa-envelope" aria-hidden="true"></i></span>
              </div>
              <input type="email" class="form-control" id="email" placeholder="Enter the Registered Email" name="Email">
            </div>
            <button type="submit" class="btn btn-primary" name="SubmitEmail">Next Step</button>
          </form>
          <!--Form End Here-->
        </div>
